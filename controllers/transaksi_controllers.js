...
export const getPeminjamanById = async (req, res) => {
  try {
    const result = await prisma.peminjama.findMany({
      where: {
...
export const borrowAnalysis = async (req, res) => {
  ...
  try {
    // Ambil barang berdasarkan kategori dan lokasi
    const items = await prisma.barang.findMany({
      where: {
        AND: [
          { category: { contains: category || "" } },
    ...
    // Analisis jumlah peminjaman dan pengembalian berdasarkan kategori barang
    const analysis = items.map(item => {
      const relevantBorrows = borrowRecords.filter(
        (record) => record.id_barang === item.id_barang
      );
